  ...
  apiKey: "AIzaSyDndwdamEqxNCXhJ1uKAOHBThYua1luCH0",
  authDomain: "blok-m-3474b.firebaseapp.com",
  projectId: "blok-m-3474b",
  storageBucket: "blok-m-3474b.firebasestorage.app",
  messagingSenderId: "1035612885691",
  appId: "1:1035612885691:web:66a4f04224010bebb0cf9e"
};
   ...
   document.addEventListener('DOMContentLoaded', () => {
        firebase.auth().onAuthStateChanged((user) => {
            if (user) {
                // UBAH DISINI: Samakan menjadi 'baksoUserBlokM'
                let savedUser = localStorage.getItem('baksoUserBlokM'); 
                if (savedUser) { 
            ...
            if (doc.exists) {
                ...
                currentUser.hp = noHp; 
                if (!currentUser.nama) currentUser.nama = "Pengguna " + noHp;
                // -----------------------------
                ...
                currentUser = { nama: "Pengguna " + noHp, role: 'kasir', hp: noHp, email: emailPalsu };
                        ...
                        // UBAH DISINI JUGA
                        localStorage.setItem('baksoUserBlokM', JSON.stringify(currentUser)); 
                        bukaLayarAplikasi();
                ...
                // UBAH DISINI JUGA
                localStorage.removeItem('baksoUserBlokM'); 
            }
        });
    });
    ...
    // 1. Sinkronkan data tanggal yang sedang aktif dengan master terbaru
    loadDataTanggalLocal(); 
    
    // 2. Render ulang tabel master produk di menu pengaturan
    renderTabelMasterProduk(); 
    
    // 3. WAJIB ADA: Render ulang tabel matriks di halaman entry stok agar ikut berubah secara real-time
    if (typeof renderTabelMatriks === 'function') {
        renderTabelMatriks();
    }
});
  ...
  function syncStokDenganMaster(tgl) { 
        if (!dbStok[tgl]) { 
            dbStok[tgl] = masterProduk.map(function(p) { 
                return Object.assign({}, p, { awal: "", tambah: "", kurang: "", sisa: "" }); 
            }); 
        } else { 
            let currentStok = dbStok[tgl]; 
            let newStokList = []; 
            masterProduk.forEach(function(mp) { 
                let found = currentStok.find(function(item) { return item.nama === mp.nama; }); 
                if (found) { 
                    newStokList.push(Object.assign({}, mp, { 
                    ...
                    newStokList.push(Object.assign({}, mp, { awal: "", tambah: "", kurang: "", sisa: "" })); 
                } 
            }); 
            dbStok[tgl] = newStokList; 
        } 

        // SIMPAN PERUBAHAN KE FIRESTORE STOK HARIAN AGAR TERSIMPAN PERMANEN
        if (typeof db !== 'undefined' && tgl) {
            db.collection('stokHarian').doc(tgl).set({ items: dbStok[tgl] }, { merge: true })
              .catch(function(err) { console.error("Gagal sinkron stok harian:", err); });
        }
    }
   ...
   function editProdukMaster(i) { 
    ...
    function hapusProdukMaster(i) { 
        if(confirm("Hapus produk dari master?")) { 
            ...
            // 3. Kirim ke Firebase dan TUNGGU sampai benar-benar tersimpan (.then)
            if(db) {
                db.collection('appData').doc('masterProduk').set({ list: masterProduk })
                  .then(() => {
                      console.log("Berhasil dihapus permanen di database.");
                      // 4. Render ulang tabel HANYA SETELAH server sukses menyimpan
                      renderTabelMasterProduk(); 
                  })
                  .catch((error) => {
...
function terapkanFilterReseller() {
    ...
    function bagikanAplikasi() {
        if (navigator.share) {
            navigator.share({
                ...
                text: "Ini link untuk mengakses Aplikasi Kasir Bakso Malang Mbak Sae'ah Cab. Blok M. Silakan buka dan simpan di HP kamu ya!",
   ...
   function catatAktivitas(aksi, detail) {
        ...
        // Memastikan currentUser.nama benar-benar ada. Jika tidak, paksa menjadi "Sistem"
        const namaUser = (typeof currentUser !== 'undefined' && currentUser && currentUser.nama) 
                            ? currentUser.nama 
                            : "Sistem";
                            
        const waktuWIB = new Date().toLocaleString('id-ID', { timeZone: 'Asia/Jakarta' });
        
        db.collection('logAktivitas').add({ 
            waktu: waktuWIB, 
            user: namaUser, 
            keterangan: `[${aksi}] ${detail}`, 
...
function cetakPDFBerkala() {
    ...
    const konfigurasiPDF = {
        margin:       0.5,
        filename:     `Laporan_Cabang_Blok M_${periode}.pdf`,
        image:        { type: 'jpeg', quality: 0.98 },
...
function bagikanWABerkala() {
    ...
    let teksWA = `*📊 LAPORAN BAKSO MBAK SAE'AH CAB. Blok M* 📊\n`;
    teksWA += `${periode}\n\n`;
